...
def _ensure_file(path: Path, label: str) -> None:
    if not path.exists():
        raise gr.Error(f"Missing {label}: {path}")
...
def _resolve_depth_checkpoint(model_type: str) -> str:
    return _resolve_repo_asset(LOCAL_MODEL_PATHS[model_type], HF_DEPTH_FILENAMES[model_type])
...
def _preload_repo_assets() -> tuple[str, ...]:
    ...
    return depth_paths + (_resolve_moge2_source(), _resolve_skyseg_path())
...
def _resolution_visibility(output_resolution_mode: str):
    return (
        gr.update(visible=output_resolution_mode == "upsample"),
        gr.update(visible=output_resolution_mode == "specific"),
        gr.update(visible=output_resolution_mode == "specific"),
    )
...
with gr.Blocks(css=CSS, theme=gr.themes.Soft()) as demo:
        ...
        with gr.Column(scale=1):
            ...
            with gr.Row():
                example_name = gr.Dropdown(
                    label="Example Data",
                    choices=[case.name for case in EXAMPLE_CASES],
                    value=EXAMPLE_CASES[0].name if EXAMPLE_CASES else None,
                )
                load_example_btn = gr.Button("Load Example")

            with gr.Accordion("Settings", open=True):
                model_type = gr.Radio(label="Mode", choices=MODEL_CHOICES, value="InfiniDepth")
            ...
            with gr.Tabs():
                with gr.Tab("3D View"):
                    model_3d = gr.Model3D(
                        ...
                        elem_id="model3d-viewer",
                    )
                with gr.Tab("Depth Comparison"):
                    depth_comparison = gr.Image(label="RGB vs Depth", type="filepath", elem_id="depth-comparison")
                with gr.Tab("Depth Maps"):
                    color_depth = gr.Image(label="Colorized Depth", type="filepath", elem_id="depth-color")
                    gray_depth = gr.Image(label="Grayscale Depth", type="filepath", elem_id="depth-preview")
                with gr.Tab("Downloads"):
                    download_files = gr.File(label="Generated Files", type="filepath")

    output_resolution_mode.change(
        fn=_resolution_visibility,
        inputs=[output_resolution_mode],
        outputs=[upsample_ratio, specific_height, specific_width],
    )
    ...
    submit_btn.click(
        fn=_clear_outputs,
        outputs=[status_output, depth_comparison, color_depth, gray_depth, model_3d, download_files],
    ).then(
        fn=_run_inference,
        inputs=[
            input_image,
